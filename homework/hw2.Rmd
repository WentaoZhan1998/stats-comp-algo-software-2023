...
The reason is that when constant is $O(10^10)$, the square will be greater than $O(10^{20})$, and the error caused by machine accuracy will be of $O(10^{-16})*O(10^{20}) = O(10^4)$, which is dominating against the true sample variance $1$. If the seed is changed, the sample mean and second moment fluctuate around 1e10 and 1e20 respectively with a small relative error, and the sample variance will fluctuate with a high relative error.

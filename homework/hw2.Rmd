...
**Answer:** I get 'sample_mean = 9999999999.97963, sample_sec_mom = 99999999999592366080, sample_var_with_question_mark = -147456'.
The estimate does not make sense.
The sample mean and second moment look well since they are around the expected value ($10^{10}$ and $10^{20}$), but the sample variance is negative, which is theoretically impossible.
The reason is that when constant is $O(10^10)$, the square will be greater than $O(10^{20})$, and the error caused by machine accuracy will be of $O(10^{-16})*O(10^{20}) = O(10^4)$, which is dominating against the truth sample variance $1$. If the seed is changed, the sample mean and second moment fluctuate around 1e10 and 1e20 respectively with a small relative error, and the sample variance will fluctuate with a high relative error.
...
**Answer:** $2$ is expected as a slope of the plotted line since the error is of magnitude $O(dx^2)$.
...
**Answer:** The first part of the line is consistent with the theory's prediction, but the latter part is not.
The derivative is actually calculated as $(f(x+dx) - f(x-dx)+eps)/2dx - f'(x) = (O(dx^3)+eps)/dx$, where $eps$ is the floating-point relative accuracy $2^{-52}$.
That means, the slope depends on the magnitudes of $dx^3$ and $eps$.
When $eps/dx^3 > o(1)$, the numerator is dominated by $dx^3$, the derivative is of rate $O(dx^2)$, and the slope is $2$; when $dx^3/eps = o(1)$, the numerator is dominated by $eps$, the derivative is of rate $O(dx^{-1})$, and the slope is roughly $-1$.
The turning point is around $-17$, i.e. $-52/3$.
...
<https://github.com/WentaoZhan1998/hiperglm/pull/1>

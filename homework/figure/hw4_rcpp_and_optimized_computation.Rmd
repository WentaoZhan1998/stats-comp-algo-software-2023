...
**Answer:**
The unexpected inner product value appears randomly in the sequence and the values are unpredictable, but the correct answer, 1023, never appears. The reason is that C++'s vector index starts from 0 instead of 1, so on the tail, i.e. $v[1023]\times w[1023]$, we are using the memory space not allocated to v and w, therefore obtaining unpredictable results. 

## Part B: Sloppy matrix-matrix multiplication

You have been told by the instructor that error-checking is particularly important in C++ and, in particular, that you should probably check for the dimension compatibixlity before you multiply two matrices in Rcpp(Eigen).
...
**Answer:**
The first run gives a $4\times2$ matrix with the first column being all 1024 and the second column being all 512. The second run gives a $4\times2$ matrix with the first column being all 1024 and the second column being unpredictable. The reason is that matrix is stored as a vector in memory. For the first column, $\mathbf{B}$ is coerced into a length $1024$ vector to time $\mathbf{A}$. However, for the second column, $\mathbf{B}$'s elements starting from $513$ are supposed to be used. Since $\mathbf{B}$ is only of length 1024, the other $512$ elements are taken from memory space outside $B$. If they are all $0$ and the result will be $512\times 1 + 512 \times 0 = 512$, otherwise the result will become unpredictable.
...
**Answer:**
The R session crushed when I run the first chunk. The reason is that the code is trying to assign value to the memory that not allocated to y. (1e3+1 to 1e7). The second chunk works because the value assignment only happens on the location where memory was assigned.
...
res_row_col_dot = row_col_dot_matmat(A, B)
res_r = A%*%B
identical(res_row_col_dot, res_r, num.eq=TRUE)
```
...
**Answer:**
Column-oriented multiplication is faster here because in the inner loop, column-oriented method is looping through a column of $\mathbf{A}$ and assigning values to a column of the result, while for the dot product, we are looping through a row of $\mathbf{A}$. Moreover, column-oriented method has a comparable (better) speed to '%*%' in R (LAPACK) (232 ms vs 410 ms).
```{r}
# Check and benchmark
res_row_col_dot = row_col_dot_matmat(A, B)
res_col_oriented = col_oriented_matmat(A, B)
identical(res_row_col_dot, res_col_oriented, num.eq=TRUE)
bench::mark(
...
**Answer:**
The results are presented below with the unit being ms. (run on JHPCE clusters)

All off: 
CXXFLAGS += -msse4 212, 4300, 949, 707. 

With one on:
CXXFLAGS += 214, 4380, 957, 706; 
CXXFLAGS += -mno-sse -mavx 114, 4360, 969, 706; 
CXXFLAGS += -mno-sse -mfma 64.6, 4350, 952, 715.

With two on: 
CXXFLAGS += -mno-sse -mavx -mfma 66.2, 4360, 956, 712; 
CXXFLAGS += -mfma 64.5, 4340, 952, 714; 
CXXFLAGS += -mavx 113, 4340, 954, 719.

With three on: 
CXXFLAGS += -mavx -mfma 65, 4370, 957, 720;

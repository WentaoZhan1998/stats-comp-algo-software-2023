...
**Your answer:** The size of the git_expreiment/ folder is 4.0M but the total file size is 75k. The reason is that there's a hidden folder .git in the repository folder. Every time we commit, the log in formation (also the old versions of files) will be saved there. If we have a large binary file to commit, the .git size will increase linearly with the committing time, taking a lot of space, and this explains why committing (large) binary files is discouraged.
